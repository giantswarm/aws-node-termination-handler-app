...
*/}}
{{- define "aws-node-termination-handler-bundle.name" -}}
...
*/}}
{{- define "aws-node-termination-handler-bundle.fullname" -}}
{{- $name := .Chart.Name -}}
...
*/}}
{{- define "aws-node-termination-handler-bundle.chart" -}}
...
*/}}
{{- define "aws-node-termination-handler-bundle.clusterID" -}}
{{- if .Values.clusterID -}}
  {{- .Values.clusterID -}}
{{- else -}}
  {{- $name := .Release.Name -}}
  {{- range $suffix := list (printf "-%s" $.Chart.Name) "-aws-node-termination-handler-bundle" -}}
...
Common labels
*/}}
{{- define "aws-node-termination-handler-bundle.labels" -}}
app.kubernetes.io/name: {{ include "aws-node-termination-handler-bundle.name" . }}
helm.sh/chart: {{ include "aws-node-termination-handler-bundle.chart" . }}
...
giantswarm.io/cluster: {{ include "aws-node-termination-handler-bundle.clusterID" . | quote }}
{{- end -}}

{{/*
Fetch crossplane config ConfigMap data
*/}}
{{- define "aws-node-termination-handler-bundle.crossplaneConfigData" -}}
{{- $clusterName := (include "aws-node-termination-handler-bundle.clusterID" .) -}}
...
*/}}
{{- define "aws-node-termination-handler-bundle.trustPolicyStatements" -}}
{{- $cmvalues := (include "aws-node-termination-handler-bundle.crossplaneConfigData" .) | fromYaml -}}
{{- $saName := default (include "aws-node-termination-handler-bundle.fullname" .) .Values.serviceAccount.name -}}
...
{{- $cmvalues := (include "aws-node-termination-handler-bundle.crossplaneConfigData" .) | fromYaml -}}
{{- $clusterID := (include "aws-node-termination-handler-bundle.clusterID" .) -}}
